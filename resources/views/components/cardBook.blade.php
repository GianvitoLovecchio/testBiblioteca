<div
    ...
    <div class="px-4 pb-4 space-y-2">
        ...
        @if (
            (Auth::check() && Auth::user()->is_admin && Route::currentRouteName() === 'search') ||
                Route::currentRouteName() === 'search.filter')
            <div>
                ...
                <p>Copie prenotate: <span
                        class="text-gray-600">{{ $book->copies->where('status', 'prenotato')->count() }}</span></p>
                <p>Copie disponibili: <span
                        class="text-gray-600">{{ $book->copies->where('status', 'disponibile')->count() }}</span></p>
            </div>
        @elseif(Auth::check() && !Auth::user()->is_admin)
            <div>
                Copie disponibili: <span
                    class="text-gray-600">{{ $book->copies->where('status', 'disponibile')->count() }}</span>
            </div>
        @endif
    </div>
    <div
